...
Exports Clipperz shorts as a DaVinci Resolve FCPXML 1.10 project. Each short
...
- ✅ Source clip on V1, alpha caption overlay on V2 — Resolve auto-detects the
  alpha and composites identically to Clipperz's baked render.
...
- ⚠️ Reframe (face-track Pan/Tilt) is **baked into the V1 source** — Resolve's
  ...
  Pan/Tilt, or re-render in Clipperz with different params.
- ⚠️ Caption animation curves (Hormozi spring physics, karaoke highlight) are
  inside the V2 alpha overlay. Swap the overlay by re-rendering in Clipperz with
...
round-trip into free Resolve and render identical to Clipperz's bake?_
...
visually identical to Clipperz's baked output.

Requires a clean cropped-but-uncaptioned intermediate from Clipperz. Today
